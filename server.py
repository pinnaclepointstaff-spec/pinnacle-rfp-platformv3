...
function getLocalFallback() {
  return {
    rfps:[
      {id:1,title:"HCBS Community Supports Waiver — Direct Support Services",agency:"Maryland Department of Health / DDA",state:"MD",type:"HCBS",value:850000,due:"2025-05-12",daysLeft:18,category:"HCBS Waiver",isNew:true,score:null,description:"Seeking qualified providers for community-based supports under Maryland's Community Supports Waiver. EVV compliance required.",competitor_avg:820000},
      {id:2,title:"Group Home Residential Services — IDD Population",agency:"Maryland DDA — Prince George's County",state:"MD",type:"Group Home",value:420000,due:"2025-05-28",daysLeft:34,category:"Residential",isNew:true,score:null,description:"Licensed Group Home operator sought for 6-bed facility serving adults with IDD.",competitor_avg:405000},
      {id:3,title:"DSP Staffing Augmentation — Therapeutic Group Homes",agency:"Virginia DBHDS",state:"VA",type:"Staffing",value:300000,due:"2025-05-20",daysLeft:26,category:"Staffing",isNew:false,score:null,description:"Qualified healthcare staffing vendor for 3 therapeutic group homes in Northern Virginia.",competitor_avg:288000},
      ...
      {id:5,title:"Supported Employment Services — DD Waiver",agency:"DC Department on Disability Services",state:"DC",type:"HCBS",value:560000,due:"2025-05-30",daysLeft:36,category:"HCBS Waiver",isNew:true,score:null,description:"DC DDS Supported Employment under DD Waiver. EVV compliance required.",competitor_avg:540000},
      {id:6,title:"Educational Paraprofessional Staffing — DCPS",agency:"DC Public Schools — Special Education",state:"DC",type:"Education",value:750000,due:"2025-06-01",daysLeft:38,category:"Education",isNew:true,score:null,description:"40+ paraprofessional positions across DCPS for 2025-26 school year.",competitor_avg:720000},
      {id:7,title:"Behavioral Health Technician Services",agency:"Virginia DBHDS — Region 4",state:"VA",type:"Staffing",value:220000,due:"2025-05-15",daysLeft:21,category:"Staffing",isNew:false,score:null,description:"Certified behavior technicians for 2 Group Homes serving adults with dual diagnoses.",competitor_avg:212000},
      {id:8,title:"Community Living Supports — Family Waiver",agency:"Maryland DDA",state:"MD",type:"HCBS",value:340000,due:"2025-06-20",daysLeft:57,category:"HCBS Waiver",isNew:false,score:null,description:"Community Living Supports through DDA's Family Supports Waiver.",competitor_avg:328000},
      {id:9,title:"Personal Supports Waiver — Home-Based",agency:"Virginia DMAS",state:"VA",type:"HCBS",value:480000,due:"2025-05-25",daysLeft:31,category:"HCBS Waiver",isNew:false,score:null,description:"Virginia DMAS personal care services for Personal Supports Waiver.",competitor_avg:462000},
      {id:10,title:"Group Home Expansion — 8-Bed Facility",agency:"Virginia DBHDS — Northern Region",state:"VA",type:"Group Home",value:290000,due:"2025-06-10",daysLeft:47,category:"Residential",isNew:false,score:null,description:"New 8-bed residential Group Home in Arlington County.",competitor_avg:278000},
      {id:11,title:"Healthcare Staff Leasing — Provider Network",agency:"DC DDS — Provider Relations",state:"DC",type:"Staffing",value:650000,due:"2025-06-25",daysLeft:62,category:"Staffing",isNew:true,score:null,description:"Staffing partnership across 5 DC residential facilities. Multi-year arrangement.",competitor_avg:625000},
      {id:12,title:"Respite Care — Family Caregiver Support Program",agency:"Maryland DDA — Montgomery County",state:"MD",type:"HCBS",value:180000,due:"2025-05-18",daysLeft:24,category:"HCBS Waiver",isNew:false,score:null,description:"Short-term respite care for families of individuals with IDD.",competitor_avg:173000},
    ],
    bids:[
...
function rfpCard(r) {
  ...
  return `<div class="rfpc" id="rc-${r.id}">
    ...
    <div class="rfpc-footer"><span class="rfpc-val">$${fmt$(r.value)}</span><span class="due-badge ${dc}">${dl}</span></div>
    <div class="rfpc-actions">
      <button class="apply-btn" onclick="openApply(${r.id})">Apply Now →</button>
...
Return JSON like: {"overall":8,"dimensions":{"geography":9,"service_match":8,"value_fit":7,"timeline":6,"competition":7,"compliance_ready":9},"summary":"2-3 sentence explanation","priority":"HIGH/MEDIUM/LOW","recommendation":"1 action sentence"}`;
  try {
    const res = await fetch(API + '/ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:500,messages:[{role:'user',content:prompt}]})});
...
Rules: Accuracy first. Flag [VERIFY: item] for unconfirmed claims. Buyer-centric language. Participant outcomes first. Structure with labeled section headers. Challenger Sale methodology where appropriate. Never conflate MD/VA/DC regulatory frameworks.`;
  try {
    ...
    const res=await fetch(API + '/ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,system:sys,messages:msgs})});
    const data=await res.json();
...
End with: OVERALL VERDICT: READY TO SUBMIT / NEEDS REVISION / DO NOT SUBMIT`;
  try {
    const res=await fetch(API + '/ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:800,messages:[{role:'user',content:prompt}]})});
    const data=await res.json();
